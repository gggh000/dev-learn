...
void * routine (void * pvoidData)  {
    ...
    cudaMalloc((void**)&dev_a, blocksPerGrid * sizeof(float));
    cudaMalloc((void**)&dev_b, blocksPerGrid * sizeof(float));
    ...
    cudaMemcpy(dev_a, a, size * sizeof(float), cudaMemcpyDeviceToHost(;
    cudaMemcpy(dev_b, b, size * sizeof(float), cudaMemcpyDeviceToHost(;
...
float alloc_test(int size, int allocType) {
    ...
    /*
    if (allocType == ALLOC_PAGE_HOST_PORTABLE) {
        cudaHostAlloc((void**)&a, size * sizeof(float), cudaHostAllocWriteCombined|cudaHostAllocMapped|cudaAllocPortable);
        cudaHostAlloc((void**)&b, size * sizeof(float), cudaHostAllocWriteCombined|cudaHostAllocMapped|cudaAllocPortable);
