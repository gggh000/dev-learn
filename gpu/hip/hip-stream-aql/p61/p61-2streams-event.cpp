...
int main (void) {
    ...
    hipLaunchKernelGGL(kernel1, blocks, threadsPerBlock, 0, stream_1, dev_a, dev_b, dev_c);
    
    // Create event that will not signal 0 until everything in stream_1 is completed (however kernel1) only.

    hipEventRecord(event, stream_1);

    // Nothing in stream_2 will run until event has signaled.    

    hipStreamWaitEvent(stream_2, event, 0);
    hipLaunchKernelGGL(kernel2, blocks, threadsPerBlock, 0, stream_2, dev_a, dev_b, dev_c);
    hipLaunchKernelGGL(kernel3, blocks, threadsPerBlock, 0, stream_1, dev_a, dev_b, dev_c);
